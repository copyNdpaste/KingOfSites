...
var updateBookmark=function(database,foldername,bmupdate,bmurl,bmname,bmimage,userid,callback){
    ...
    database.FolderModel.findOneAndUpdate({userid:userid,foldername:foldername,bmname:bmupdate},{$set:{bmname:bmname,bmurl:bmurl,bmimage:bmimage}}/*,{new:true}*//*new:true는 변경된 사항 return함.*/,function(err,results){ 
        console.log('정보 수정1');
        if(err){
            callback(err,null);
    ...
    try{
        ...
        if(database){
                ...
                if(results){
                    ...
                    console.log(results);
                    res.render('updatedsite',{results:results,user:user});
                }else{
